...
class Condition(object):
    ...
    def encode(self, pipe: FluxPipeline) -> Tuple[torch.Tensor, torch.Tensor, int]:
        ...
        # check the position delta and position scale
        has_delta = self.position_delta is not None and self.position_delta != [0, 0]
        has_scale = self.position_scale != 1.0
        print(has_delta, has_scale)
        assert (
            not has_delta or not has_scale
        ), "Position delta and scale cannot be used together!"
